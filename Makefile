...
# BITCLOCK = 5.33 is 187.5 kHz — ignored by old USBasp firmware.
# Old USBasp defaults to 375 kHz which is too fast for a fresh chip
# running on the factory 1 MHz internal RC oscillator (ISP max = 250 kHz).
# Fix: close JP3 jumper on USBasp (forces 8 kHz), OR update USBasp firmware.
# After fuses are set (16 MHz crystal active) JP3 can be removed.
BITCLOCK   = 5.33
PROGRAMMER = usbasp
PORT       = usb

# Fuses for ATmega32A @ 16 MHz external crystal.
# LFUSE 0xFF: CKSEL=1111 (ext crystal >8 MHz), SUT=11 (max startup), BOD off.
# HFUSE 0xC9: CKOPT=0 (full-swing oscillator, required for 16 MHz),
#             JTAGEN=1 (JTAG disabled — frees PC0-PC7 for DAC0808),
#             SPIEN=0 (ISP enabled), OCDEN=1, BOOTRST=1.
# ATmega32A has no EFUSE byte.
LFUSE    = 0xFF
HFUSE    = 0xC9
...
# Set the fuse bytes (requires JP3 closed on USBasp if chip is fresh/1 MHz)
fuse:
	@echo "Setting fuses (ATmega32A, 16 MHz external crystal)..."
	$(AVRDUDE) -p $(MCU) -c $(PROGRAMMER) -P $(PORT) -B $(BITCLOCK) \
		-U lfuse:w:$(LFUSE):m \
		-U hfuse:w:$(HFUSE):m
	@echo "Fuses set successfully."

# Flash + fuse in one go using slow clock (use this on a fresh chip with JP3 closed)
program-slow:
	@echo "Flashing at slow clock (for fresh chip / old USBasp firmware)..."
	$(AVRDUDE) -p $(MCU) -c $(PROGRAMMER) -P $(PORT) \
		-U lfuse:w:$(LFUSE):m \
		-U hfuse:w:$(HFUSE):m \
		-U flash:w:$(BUILD_DIR)/$(TARGET).hex
	@echo "Done. Remove JP3 jumper from USBasp for normal speed."
...
# Auto-include header dependency files generated by -MMD
-include $(OBJS:.o=.d)

.PHONY: all build size clean program fuse program-slow
